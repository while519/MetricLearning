...
def SGDexp(state):
    ...
    trainfunc = TrainFn3Member(apply_fn, mapping, state.marge)
    ...
    tmpNP = np.zeros(shape=(state.nsamples, state.neval), dtype=np.int32)
    ...
    for epoch_count in range(1, state.totepochs + 1):
        ...
        trainIdxrn = np.random.choice(state.nsamples, state.ntrain + state.nsamples, replace=True, p=None)

        for ii in range(state.nsamples):
            trainIdxl = np.append(trainIdxl, [ii])
            trainIdxr = np.append(trainIdxr, [np.random.choice(state.nsamples, 1, p=Q[ii, :])])
