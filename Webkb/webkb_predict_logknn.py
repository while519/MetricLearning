...
ch_file = logging.FileHandler(filename= 'predict_' + applyfn + str(outdim) + '_logknn' +
...
def SGDexp(state):
    ...
    trainfunc = TrainFn6Member(apply_fn, mapping, P)
...
if __name__ == '__main__':
    ...
    simi_X = consine_simi(X) + 1e-8
    np.fill_diagonal(simi_X, 1e-8)
    ...
    dist_X = np.ones(shape=(state.nsamples, state.nsamples))*np.log(5.)
    knnIdx = np.argsort(-simi_X.copy(), axis=1)[:,:knn]
    for rowind in range(state.nsamples):
        dist_X[rowind, knnIdx[rowind,:]] = 0.

    P = T.as_tensor_variable(np.asarray(dist_X, dtype=theano.config.floatX))
